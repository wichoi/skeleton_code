...
int json_manager::config_create(list<json_param> &obj_list, string &json_body)
{
    ...
    if(json_obj != NULL)
    {
        while(obj_list.size() > 0)
        {
            ...
            else if(data.type == json_param::TYPE_FLO)
                cJSON_AddNumberToObject(json_obj, data.key.c_str(), data.value_fl);
        ...
        if(root != NULL)
        {
            ...
            for(iter = obj_list.begin(); iter != obj_list.end(); ++iter)
            {
                ...
                if(result != NULL)
                {
                    ...
                    else if(iter->type == json_param::TYPE_FLO)
                    {
                        log_d("%s [%lf] \n", str.c_str(), result->valuedouble);
                        iter->value_fl = result->valuedouble;
                    }
                }
            }
        }
