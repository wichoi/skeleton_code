...
// cluster id
// 0x0402 - ZCL_TEMP_MEASUREMENT_CLUSTER_ID

// server

// attribute id
// 0x0000 (M) - MeasuredValue (s16)         ZCL_TEMP_MEASURED_VALUE_ATTRIBUTE_ID
// 0x0001 (M) - MinMeasuredValue (s16)      ZCL_TEMP_MIN_MEASURED_VALUE_ATTRIBUTE_ID
// 0x0002 (M) - MaxMeasuredValue (s16)      ZCL_TEMP_MAX_MEASURED_VALUE_ATTRIBUTE_ID
// 0x0003 (O) - Tolerance (u16)             ZCL_TEMP_TOLERANCE_ATTRIBUTE_ID
...
int apro_zcl_temp_attr(u16 net_id, u8 ep, zb_frame_t *frame)
{
    ...
    frame->cluster_id = ZCL_TEMP_MEASUREMENT_CLUSTER_ID;
    frame->src_ep = 1;
    frame->dest_ep = ep;
    ...
    frame->msg[frame->msg_len++] = ZCL_TEMP_MEASURED_VALUE_ATTRIBUTE_ID >> 8;
    frame->msg[frame->msg_len++] = ZCL_TEMP_MEASURED_VALUE_ATTRIBUTE_ID;
            ...
            if(payload.cnt > 0)
            {
                ...
                for(i = 0; i < payload.cnt; i++)
                {
                        ...
                        payload.field[i].state == EMBER_ZCL_STATUS_SUCCESS)
                    {
                        ...
                        s16 tem;
                        memcpy((char*)&tem, (char*)&payload.field[i].data[0], sizeof(tem));
                        // payload.field[i].data_type == ZCL_INT16S_ATTRIBUTE_TYPE
                        log_i("Temperature %u\n", tem);
                        ...
                        ocf_pay.cmd = *cmd;
                        memcpy((char*)&ocf_pay.data[0], (char*)&tem, sizeof(tem));
                        ocf_pay.data_len = sizeof(tem);
