...
char *jRead_strcpy( char *destBuffer, int destLength, struct jReadElement *pElement )
{
	...
	if( pElement->error == 0 )
	{
		if( len >= destLength ){
			//len = destLength;
			len = destLength -1; // kkko, modify destLengt
		}
		
		//for( i=0; i<destLength; i++ )
		for( i=0; i<len; i++ ){ // kkko,
			*pdest++= *psrc++;
		}
	}
	*pdest= '\0';
...
int jRead_int_def(char *pJson, char *pQuery, int *queryParams, int def_val)
{
	struct jReadElement elem;
	long result;
	jReadParam(pJson, pQuery, &elem, queryParams);
...
int jRead_int_neg(char *pJson, char *pQuery, int *queryParams)
{
	return jRead_int_def(pJson, pQuery, queryParams, -1);
}
...
//-------------------------------------------------
// ++ Helper function
int jGet_long( char *pJson, char *pQuery, long *val )
{
	struct jReadElement elem;
	jReadParam( pJson, pQuery, &elem, NULL );
	if( (elem.dataType == JREAD_ERROR) || (elem.dataType == JREAD_NULL))
		return -1;
	if( elem.dataType == JREAD_BOOL )
	{
		*val = *((char *)elem.pValue)=='t' ? 1 : 0;
		return 0;
	}

	jRead_atol( (char *)elem.pValue, val );
	return 0;
}

int jGet_int( char *pJson, char *pQuery, int *val )
{
	long l_val = *val;
	int res = jGet_long( pJson, pQuery, &l_val );
	*val = (int)l_val;
	return res;
}

int jGet_double( char *pJson, char *pQuery, double *val )
{
	struct jReadElement elem;
	jReadParam( pJson, pQuery, &elem, NULL );
	if( elem.dataType == JREAD_ERROR )
		return -1;
	jRead_atof( (char *)elem.pValue, val );
	return 0;
}
